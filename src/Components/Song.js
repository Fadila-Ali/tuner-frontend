...
export default function Song({ song }) {
  ...
  return (
    <tr class="bg-white border-b dark:bg-gray-50 dark:border-gray-100 transition duration-300 ease-in-out hover:bg-teal-100">
      ...
      <td class="md:px-8 md:py-4 sm:px-4">
        <Link to={`/songs/${song.id}`} className="hover:text-purple-500">
          {song.name}
        </Link>
      </td>
      <td class="md:px-8 md:py-4 sm:px-4">
      ...
      <td class="md:px-8 md:py-4 sm:px-4">
        {song.is_favorite ? (
          <span>
            <FaStar className="text-teal-500 hover:text-purple-500" />
          </span>
        ) : (
          <span>
            <BiStar className="text-teal-500 hover:text-purple-500" />
          </span>
        )}
      </td>
    </tr>
